...
const RideDetailsModal = ({ ride, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md animate-in fade-in duration-300">
      {/* Max width increase ki hai (max-w-4xl) taake split layout acha lage */}
      <div className="bg-white w-full max-w-4xl rounded-[3rem] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 flex flex-col md:flex-row min-h-[500px]">
        
        {/* LEFT COLUMN: Rider & Route (Dark Theme) */}
        <div className="w-full md:w-[45%] bg-slate-900 p-10 text-white flex flex-col justify-between relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex justify-between items-start mb-12">
              ...
              <div className="bg-white/10 px-3 py-1 rounded-full backdrop-blur-md">
                 <p className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest">{ride.status}</p>
              </div>
            </div>
            ...
            <div className="space-y-10 relative before:content-[''] before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-[2px] before:bg-gradient-to-b before:from-emerald-500 before:to-slate-700">
              <div className="relative pl-10">
                ...
                <p className="font-bold text-slate-100">{ride.startStationId?.name || "Starting Point"}</p>
                <p className="text-[11px] text-slate-500">{moment(ride.startTime).format("ddd, MMM DD • hh:mm A")}</p>
              </div>
              ...
              <div className="relative pl-10">
                ...
                <p className="font-bold text-slate-100">
                    {ride.endStationId?.name || (ride.status === "active" ? "Ongoing Trip..." : "Trip Ended")}
                </p>
                {ride.endTime && (
                   <p className="text-[11px] text-slate-500">{moment(ride.endTime).format("ddd, MMM DD • hh:mm A")}</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="mt-8 pt-8 border-t border-white/5 flex items-center gap-2 text-slate-500">
            <ShieldCheck size={16} />
            <span className="text-[10px] font-medium tracking-wide italic">Verified Ride Log by Volt-26 Systems</span>
          </div>
        </div>
        ...
        <div className="w-full md:w-[55%] p-10 flex flex-col">
          ...
          <div className="flex-1 space-y-8">
            <div>
              <h3 className="text-xs font-black uppercase text-slate-400 tracking-widest mb-4 flex items-center gap-2">
                <Bike size={16} className="text-emerald-500" /> Vehicle Specification
              </h3>
              
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-50 p-5 rounded-[2rem] border border-slate-100">
                  <p className="text-[10px] font-black text-slate-400 uppercase">Model</p>
                  <p className="text-base font-black text-slate-900">{bikeDetails?.model_name || "Volt-26 Standard"}</p>
                </div>
                <div className="bg-slate-50 p-5 rounded-[2rem] border border-slate-100">
                  <p className="text-[10px] font-black text-slate-400 uppercase">Registration</p>
                  <p className="text-base font-black text-slate-900">{bikeDetails?.registration_number || "PZ-XXXX"}</p>
                </div>
              </div>
              ...
              <div className="mt-4 flex gap-4">
                <div className="flex-1 bg-emerald-50/50 p-4 rounded-2xl flex items-center gap-3">
                   ...
                   <div>
                     <p className="text-[10px] font-bold text-emerald-600 uppercase leading-none">Battery</p>
                     <p className="text-sm font-black text-slate-800">{bikeDetails?.battery_level || 100}%</p>
                   </div>
                </div>
                <div className="flex-1 bg-blue-50/50 p-4 rounded-2xl flex items-center gap-3">
                   ...
                   <div>
                     <p className="text-[10px] font-bold text-blue-600 uppercase leading-none">Hourly Rate</p>
                     <p className="text-sm font-black text-slate-800">Rs. {bikeDetails?.price_per_hour || 100}</p>
                   </div>
                </div>
              </div>
            </div>
            ...
            <div>
              <h3 className="text-xs font-black uppercase text-slate-400 tracking-widest mb-4 flex items-center gap-2">
                <Clock size={16} className="text-emerald-500" /> Financial Summary
              </h3>
              <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden">
                ...
                <div className="flex justify-between items-center mb-6">
                   ...
                   <span className="text-sm font-bold">
                     {ride.endTime ? `${moment.duration(moment(ride.endTime).diff(moment(ride.startTime))).asMinutes().toFixed(0)} Minutes` : "Ongoing"}
                   </span>
                </div>
                <div className="border-t border-white/10 pt-6 flex justify-between items-end">
                   <div>
                     <p className="text-[10px] font-black text-emerald-400 uppercase mb-1">Grand Total</p>
                     <p className="text-4xl font-black">Rs. {ride.totalCost || 0}</p>
                   </div>
                   <div className="text-right">
                     <p className="text-[10px] text-slate-400 mb-1">Paid via Wallet</p>
          ...
          <button
            onClick={onClose}
            className="mt-8 w-full py-5 bg-slate-900 text-white rounded-[1.5rem] font-black uppercase tracking-widest text-xs hover:bg-emerald-500 transition-all shadow-xl active:scale-[0.98]"
          >
            Acknowledge Receipt
          </button>
        </div>
